...
from utils.data_loading import AugDataset
...
dir_img = Path('C:/Users\jimbo\Documents\PhD stuff\Segmentation models/3D processing\Training set/Images/')
dir_mask = Path('C:/Users\jimbo\Documents\PhD stuff\Segmentation models/3D processing\Training set/Masks/')
dir_checkpoint = Path('C:/Users\jimbo\Documents\PhD stuff\Segmentation models/3D processing\Training set/CPs')
...
def train_model(
    ...
    dataset = AugDataset(dir_img, dir_mask, img_scale)
    ...
    for epoch in range(1, epochs + 1):
        ...
        if save_checkpoint:
            ...
            files = glob.glob(str(dir_checkpoint / '*.pth'), recursive=True)
            for f in files:
                os.remove(f)
...
if __name__ == '__main__':
    ...
    # model = smp.UnetPlusPlus(in_channels=3, classes=args.classes, encoder_name='densenet161',
    ...
    # state_dict = torch.load(os.getcwd() + '/checkpoint_epoch158.pth', map_location=device)
